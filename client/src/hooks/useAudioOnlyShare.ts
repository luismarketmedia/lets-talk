...
export const useAudioOnlyShare = () => {
  ...
  const [audioShareStream, setAudioShareStream] = useState<MediaStream | null>(
    null,
  );
  ...
  const startAudioShare = useCallback(
    async (options: AudioOnlyShareOptions = {}) => {
      try {
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to start audio sharing";
  ...
  const applyAudioEffects = useCallback(
    (effects: { gain?: number; lowpass?: number; highpass?: number }) => {

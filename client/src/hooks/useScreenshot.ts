...
export const useScreenshot = () => {
  ...
  const takeElementScreenshot = useCallback(
    async (
      element: HTMLElement,
      filename?: string,
      format: "png" | "jpeg" = "png",
  ...
  const takeVideoGridScreenshot = useCallback(
  ...
  const takeFullCallScreenshot = useCallback(
    async (filename?: string) => {
      const callInterface = document.querySelector(
        "[data-call-interface]",
      ) as HTMLElement;
      if (!callInterface) {
  ...
  const takeAndDownloadScreenshot = useCallback(
    async (
